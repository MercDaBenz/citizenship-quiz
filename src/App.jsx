...
export default function App() {
  ...
  function sendSMS() {
    const done    = history.filter(r => !r.pending);
    const wrongQs = done.flatMap(r => r.questions.filter((_, qi) => !r.answers[qi]?.correct));
    const list    = wrongQs.length
      ? wrongQs.map((q, i) => `${i+1}. ${q.question}`).join("\n")
      : "You got everything right! Keep reviewing.";
    const msg = encodeURIComponent(`🗽 Citizenship Test Reminder!\n\nPractice these:\n${list}\n\nGood luck!`);
